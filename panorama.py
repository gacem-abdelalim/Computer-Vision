...
def ransac(keypoints1, keypoints2, matches, n_iters=500, threshold=1):
    ...
    for i in range(n_iters):
        ...
        #*******************************************************************
        points1 = keypoints1[matches1[:,0]]
        pointsHomogene = np.column_stack((points1, np.ones(len(points1))))

        points1Transf = np.dot(H1, pointsHomogene.T)
        
        # Normaliser les coordonnées homogènes pour revenir à des coordonnées cartésiennes
        points1Transf = (points1Transf / points1Transf[2])[:2].T
        #**************************************************************
        ...
        distances = np.linalg.norm(points1Transf - actual_coords, axis=1)
        distances[indx] = 20000

        inlier_indices = np.where(distances <= 1)[0] #np.sqrt(2*(threshold**2))
