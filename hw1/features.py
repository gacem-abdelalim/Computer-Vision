...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        # TODO-BLOC-DEBUT
        I_X = ndimage.sobel(srcImage,axis=0,mode='reflect')
        I_Y = ndimage.sobel(srcImage,axis=1,mode='reflect')
        
        A = ndimage.gaussian_filter(I_X**2, sigma=0.5, mode='reflect')
        B = ndimage.gaussian_filter(I_X*I_Y, sigma=0.5, mode='reflect')
        C = ndimage.gaussian_filter(I_Y**2, sigma=0.5, mode='reflect')
        
        k = 0.1
        mat_H = np.array([[A,B],[B,C]])
        det_H = A * C - B ** 2
        trace_H = A + C
    
        harrisImage = det_H - k * trace_H ** 2
    ...
    def computeLocalMaxima(self, harrisImage):
        ...
        # TODO-BLOC-DEBUT
        #harrisImage = ndimage.gaussian_filter(harrisImage, sigma=0.5, mode='reflect')
        
        
        maxima = np.zeros_like(harrisImage, bool)
        xd = np.zeros_like(harrisImage, bool)
        filtre = np.ones((7, 7))
        xd = harrisImage > 0.001  
        maxima = scipy.ndimage.maximum_filter(harrisImage, footprint=filtre, mode='reflect') == harrisImage
        destImage = np.logical_and(maxima, xd)
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            # TODO-BLOC-DEBUT  
            if (i==0):
                gray = cv2.copyMakeBorder(grayImage, 2, 2, 2, 2, cv2.BORDER_CONSTANT, value=0)
            
            descMatrix = gray[y:y+5, x:x+5]
            
            desc[i] = descMatrix.flatten()
...
class MOPSFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            # TODO-BLOC-DEBUT
            Moy = np.mean(destImage)
            Ecart_type = np.std(destImage)
            if np.abs(Ecart_type) < 1e-10 :
                desc[i] = np.zeros((8,8)).flatten()
            else:
                destImage = (destImage - Moy) / Ecart_type
...
class SSDFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        """Remarque
            queryIdx--->i  (l'indice du keypoint dans image1)
            trainIdx--->j  (l'indice du keypoint dans image2)
            distance--> EuclMatrix[i,j] """
        ...
        MatrixImg1 = scipy.spatial.distance.cdist(desc1, desc2, 'euclidean')
        MatrixImg2 = scipy.spatial.distance.cdist(desc2, desc1, 'euclidean')
        ...
        for i in range(v):
            j = np.argmin(MatrixImg1[i])
            d_sorted = np.sort(MatrixImg1[i])
            f = cv2.DMatch()
            f.queryIdx = i
            f.trainIdx = j
            f.distance = d_sorted[0]
            cordinateImg1.append((i,j))
            matchesImg1.append(f)

        for i in range(k):
            j = np.argmin(MatrixImg2[i])
            cordinateImg2.append((j,i))

        m = 0
        for cordinate in cordinateImg1:
            if cordinate in cordinateImg2:
                matches.append(matchesImg1[m])
                m += 1
            else:
                m += 1
...
class RatioFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        Entrée :
            ...
            desc2 -- les descripteurs de primitives de l'image 2 stockés dans un tableau numpy,
                dimensions:   lignes   (nombre de points-clés)
                             colonnes (dimension du descripteur)
        Sortie :
            liste des correspondances : une liste d'objets cv2.DMatch
                Comment définir les attributs :
                    queryIdx : L'index de la primitive dans la première image
        ...
        if v>k:
            for i in range(v):
                if i<k:
                    j = np.argmin(MatrixImg1[i])
                    d_sorted = np.sort(MatrixImg1[i])
                    ratio = d_sorted[0]/d_sorted[1]
                    if (ratio <0.7 ):
                        f = cv2.DMatch()
                        f.queryIdx = i
                        f.trainIdx = j
                        f.distance = d_sorted[0]/d_sorted[1]
                        cordinateImg1.append((i,j))
                        matchesImg1.append(f)
                    
                    j = np.argmin(MatrixImg2[i])
                    d_sorted2 = np.sort(MatrixImg2[i])
                    ratio = d_sorted2[0]/d_sorted2[1]
                    if (ratio <0.7 ):
                        cordinateImg2.append((j,i))
